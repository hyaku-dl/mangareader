...
if os.name != "posix":
    ...
    CFLOP = [
        rf"{os.getcwd()}\mrt.yml",
        rf"{os.getenv('USERPROFILE')}\AppData\Roaming\mrt\config.yml",
    ]
    ...
    POSIX = 1
    CFLOP = [f"{os.getcwd()}/mrt.yml", "~/.config/mrt/config.yml", "~/.mrt"]
    if xch := os.getenv("XDG_CONFIG_HOME"):
        CFLOP.insert(1, f"{xch}/mrt/config.yml")
